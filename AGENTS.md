...
Personal German-practice app (A1→B1), Quizlet/Blooket style: fast gamified drilling of verbs + conjugation, nouns with der/die/das gender, and adjectives, plus a browsable Tips mode for the rules. Single user, no account, no backend — progress lives on the device.

Full plan (v1 scope, content model, quiz modes, roadmap): [`docs/PLAN.md`](docs/PLAN.md).
...
- **Layers**: Screens → ViewModels/Controllers → Services → Models. Data loading goes through `lib/data/`, never straight from a widget.
- **No business logic in widgets.** Widgets render state and dispatch events.
- **Content is data, not code.** Vocabulary and rules live in JSON assets; adding content never requires editing Dart.
- **Single source of truth for types**: domain models in `lib/models/`; JSON is parsed once at the boundary into those models.
- **Minimum viable everything.** A deliberate simplification is marked with a `ponytail:` comment naming its ceiling and upgrade path.

## Code quality rules (MANDATORY)

Canonical source: [`standards/code-quality.md`](standards/code-quality.md). Summary:

- One public symbol per file (one widget / model / service).
- File size limits — widget 150, screen 200, service 150, model 120.
- Function 30 lines max, complexity 10 max, nesting 3 max.
- Guard clauses + early returns. No `else` after `return`.
- No `dynamic` without a parse guard at the JSON boundary.
- Errors typed and surfaced; never swallowed.
- Tests in `test/`, mirroring the `lib/` path.

Enforced via `flutter analyze` (`analysis_options.yaml`, `flutter_lints`). The canonical `standards/code-quality.md` is written for a TS stack; where it names TS-only tooling, the Dart equivalents above apply (recorded in `docs/DEVIATIONS.md`).
...
lib/
  main.dart
  models/      # domain entities parsed from JSON
  data/        # asset loading + persistence
assets/        # static JSON content
test/          # mirrors lib/
docs/          # see Documentation map
standards/     # code-quality core
```
...
| `docs/PLAN.md` | v1 development plan — scope, content model, quiz modes, roadmap |
| `docs/decisions/` | ADRs |
| `docs/audits/` | Point-in-time role audits (immutable) |
...
| Role(s) | Human owner |
|---------|-------------|
| Every role, sponsor included | session user (personal single-owner project) |

Single-owner project: deliverables chain within the session, no external approval gate. If a role is unmapped, the agent asks once and records the answer here — the question must never repeat. Keep this table current via PR when the team changes.
